...
export function loadConfig(): GatewayConfig {
  return {
    ...
    glmNativeBaseUrl: process.env.GLM_NATIVE_BASE_URL?.trim() || 'https://open.bigmodel.cn/api/paas/v4',
    glmApiKey: required('GLM_API_KEY'),
    models: {
      vision: process.env.GLM_VISION_MODEL?.trim() || 'glm-4.6v',
      image: process.env.GLM_IMAGE_MODEL?.trim() || 'cogview-4',
    },
    coverFontPath: process.env.COVER_FONT_PATH?.trim() || '/usr/share/fonts/truetype/msyh/msyh.ttc',
  };
}
