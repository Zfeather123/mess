...
describe('wrapCjk:封面标题一字不差(JIN-57 回归门禁)', () => {
  /**
   * ⚠️ 已知缺陷(QA 复现,待修):**容量够却把标题截断丢字**。
   *
   * 生产默认值 maxChars=8 / maxLines=3 = 24 字容量,这个标题只有 21 字,本该放得下。
   * 实际输出:「被辞退后公司不给 / 赔偿金, / 三万块该怎么要…」—— 「回来」被吃掉了。
   *
   * 成因:折行按语义段(标点切)装行,段落一旦超过 maxChars 就被 hardSplit 直接推进
   * lines,**行尾剩余空间不再回填**,于是每个超长段都浪费半行;行数被撑到 4 > maxLines=3,
   * 最后一行再被 slice + '…' 截断 —— 截掉的是真字符。
   *
   * 而 composeCover 的文档契约白纸黑字写着「标题一字不差…绝不截断文案」。契约被违反。
   * 封面标题是内容的脸,少两个字没人会发现,但文案就废了。
   *
   * 修好之后:去掉 `.fails`,这条会变绿。
   */
  it.fails('【已知缺陷】普通标题:折行不丢字、不改字、不重排', () => {
  ...
  /**
   * ⚠️ 已知缺陷(同上,另一个触发面):连续收尾标点 + 段落装行浪费 → 提前触顶 maxLines → 丢字。
   *
   * 19 字标题、24 字容量,输出「加班费、/ 年假、/ 赔偿金,/ 一个都别放…」——「过!!」没了。
   * 前三行分别只用了 4/3/4 格(共 6 格),空转了一半的版面却仍然截断。
   */
  it.fails('【已知缺陷】连续标点结尾的标题:容量足够时也不该丢字', () => {
