...
| 点数余额 / 充值 / 流水 | ✅ **复用 JIN-50 的 0148**:`compute_accounts` / `compute_transactions` / `compute_recharge_orders` |
| **冻结明细 + 超时回收** | ➕ 新增 `compute_reservations`(0149)|

> ⚠️ **不要再起一套 `credit_*` 表。** 0148 已经把算力账户建好了 —— `compute_accounts`
> 甚至已经有 `frozen_points` 和乐观锁 `version`。另起一套 = 同一个概念两份真相。
> 我第一版就是这么写的,合到 `main` 才发现撞车,已改为绑定 `compute_*`。

`budget_policies` 是"花超了拦"(允许透支到超出那一次),点数制是"不够就不让发"(一分不能透支)。
语义不同,不能拿它顶。

### 超时预留回收(sweeper)—— 已实现,不是 TODO

网关进程若在 `reserve` 之后、`settle` 之前被 kill,这笔冻结会永远挂在 `held` ——
**用户的点数凭空消失,且账上查不出原因**。这是两阶段扣费自带的失效模式,必须闭环。

`packages/billing/src/sweeper.ts`:定时扫 `state='held'` 且超过 TTL 的预留,一律 `release`。

- TTL 默认 15 分钟(`BILLING_RESERVATION_TTL_MS`,配置项)
- **幂等**:sweeper 和 settle 会撞车 —— sweeper 扫到超时 held 的同一刻,
  卡住的请求可能刚好活过来要 settle。两边都动余额 = **双花**。
  靠 `held`-only 的原子状态推进兜住:谁先把状态推离 `held` 谁生效,另一边变 no-op。
- 单测覆盖:reserve → 进程死 → sweeper 回收 / sweeper 与 settle 撞车 / 重复扫不多退钱

落库:0149 的 `compute_reservations`(`(state, expires_at)` 上有索引给 sweeper 走)。
...
| `GLM_OPENAI_BASE_URL` | `https://open.bigmodel.cn/api/paas/v4` | 原生 OpenAI 端点,**视觉工具专用** |
| `GLM_OPENAI_API_KEY` | (必填) | 同上 |
| `GLM_VISION_MODEL` | `glm-4.6v` | 读图。实测满分 |
| `GLM_IMAGE_MODEL` | `cogview-4` | 出底图 |
| `JIN_COVER_FONT_PATH` | — | 封面中文字体。**缺了会渲染成豆腐块** |
...
JIN_COVER_FONT_PATH=/path/to/msyh.ttc \
  pnpm --filter @xiaojing/gateway test
```
